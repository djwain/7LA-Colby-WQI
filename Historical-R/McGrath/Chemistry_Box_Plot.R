...
png(file="TP1_Cond_box.png", units="in",width=5, height=5.5, res=400)

# Make box plot by year and by site
p <- ggplot(data = S, aes(x = Dec,y = `Conductivity (uS)`)) +
  geom_boxplot() +
  labs(title = "Togus Pond Conductivity by Decade", y = 'Cond (uS)',x = '')
print(p)
dev.off()

png(file="TP1_Alk_box.png", units="in",width=5, height=5.5, res=400)

# Make box plot by year and by site
p <- ggplot(data = S, aes(x = Dec,y = `Alkalinity (mg/L)`)) +
  geom_boxplot() +
  labs(title = "Togus Pond Alkalinity by Decade", y = 'Alk (mg/L)',x = '')
print(p)
dev.off()
