...
# Find epicore data
PC <- Pall %>% filter(Depth <= 1 & `Sample Type` != 'C')
...
splot <- Pavg
mk <- mk1
png(file=paste(site, "_TP_SG.png",sep = ""), units="in",width=5, height=5.5, res=400)
...
plot(splot$Year,splot$`Total P`, las=1, ylim=c(0,yhigh),ylab="Average TP - SG (ppb)",xlab="Year")  
...
splot <- Pavg10
mk <- mk10
png(file=paste(site, "_TP_SG10.png",sep = ""), units="in",width=5, height=5.5, res=400)
...
png(file=paste(site, "_TPSG_all.png"), units="in",width=10, height=5, res=400)
plot(Pavgd$Year,Pavgd$`Total P`,ylim=c(0,yhigh),pch=21,col = 'azure3',bg = 'azure3',
     ylab="TP - SG (ppb)",xlab="Year")  
mtext(text=paste(titles, "\n", " (MIDAS ",titles2," - Station ",titles3,")",sep = ""), side=3, line=1.9, font=2)
points(Pavg$Year,Pavg$`Total P`,ylim=c(0,yhigh),pch=21,cex = 1.5,bg = 'aquamarine3',ylab="TP - SG (ppb)",xlab="Year")
dev.off()

# Make box plot by decade
png(file=paste(site, "_PSG_box.png"), units="in",width=5, height=5.5, res=400)
S1 <- Pavgd
p <- ggplot(data = S1, aes(x = Dec,y = `Total P`)) +
  geom_boxplot() +
  labs(title = paste(titles, "\n", " (MIDAS ",titles2," - Station ",titles3,")",sep = ""), y = "TP - SG (ppb)",x = '')
print(p)
dev.off()
