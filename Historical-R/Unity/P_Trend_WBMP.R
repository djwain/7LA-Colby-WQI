...
PG <- Pall %>% filter(`Sample Type` == 'G')
outfilename <- paste(site,"_TP_grab.csv",sep = "")
names(PG) <- make.names(names(PG))
write.csv(PG, outfilename, row.names = FALSE)
...
splot <- Pavg
mk <- mk1
png(file=paste(site, "_TP_EC.png",sep = ""), units="in",width=5, height=5.5, res=400)
...
splot <- Pavg10
mk <- mk10
png(file=paste(site, "_TP_EC10.png",sep = ""), units="in",width=5, height=5.5, res=400)
...
png(file=paste(site, "_TP_all.png"), units="in",width=10, height=5, res=400)
plot(Pavgd$Year,Pavgd$`Total P`,ylim=c(0,yhigh),pch=21,col = 'azure3',bg = 'azure3',
     ylab="TP - EC (ppb)",xlab="Year")  
mtext(text=paste(titles, "\n", " (MIDAS ",titles2," - Station ",titles3,")",sep = ""), side=3, line=1.9, font=2)
...
png(file=paste(site, "_P_box.png"), units="in",width=5, height=5.5, res=400)
S1 <- Pavgd
p <- ggplot(data = S1, aes(x = Dec,y = `Total P`)) +
  geom_boxplot() +
  labs(title = paste(titles, "\n", " (MIDAS ",titles2," - Station ",titles3,")",sep = ""), y = "TP - EC (ppb)",x = '')
print(p)
dev.off()
