...
Secchi <- Secchi %>% filter(MONTH >= 7 & MONTH <= 8)
...
splot <- Savg
mk <- mk1
png(file=paste(site, "_JASDT.png",sep = ""), units="in",width=5, height=5.5, res=400)
...
plot(Savg$YEAR,Savg$`SECCHI DEPTH`, las=1, ylim=c(0,yhigh),ylab="Average Jul-Aug SDT (m)",xlab="Year")  
...
splot <- Savg10
mk <- mk10
png(file=paste(site, "_JASDT10.png",sep = ""), units="in",width=5, height=5.5, res=400)
...
plot(Savg10$YEAR,Savg10$`SECCHI DEPTH`, las=1, ylim=c(0,yhigh),ylab="Average Jul-Aug SDT (m)",xlab="Year")  
...
png(file=paste(site, "_JASDT_all.png",sep = ""), units="in",width=10, height=5, res=400)
plot(Savgd$YEAR,Savgd$`SECCHI DEPTH`,ylim=c(yhigh,0),pch=21,col = 'azure3',bg = 'azure3',
     ylab="Jul-Aug SDT (m)",xlab="Year")  
mtext(text=paste(titles, "\n", " (MIDAS ",titles2," - Station ",titles3,")",sep = ""), side=3, line=1.9, font=2)
points(Savg$YEAR,Savg$`SECCHI DEPTH`,ylim=c(yhigh,0),pch=21,cex = 1.5,bg = 'aquamarine3',ylab="Jul-Aug SDT (m)",xlab="Year")
#lines(lowess(Savg$YEAR,Savg$`SECCHI DEPTH`),col=4)
dev.off()

# Plot secchi box plots by decade
png(file=paste(site, "_JASDT_box.png",sep = ""), units="in",width=5, height=5.5, res=400)
S1 <- Savgd
p <- ggplot(data = S1, aes(x = Dec,y = `SECCHI DEPTH`)) +
  geom_boxplot() +
  labs(title = paste(titles, "\n", " (MIDAS ",titles2," - Station ",titles3,")",sep = ""), y = 'Jul-Aug SDT (m)',x = '')
print(p)
dev.off()
