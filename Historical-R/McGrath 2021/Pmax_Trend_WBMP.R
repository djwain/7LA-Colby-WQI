...
Pyr1 <- P %>% filter(Station == 1) %>% group_by(Year) %>% summarise(yrmean = max(`Total P`))
...
yhigh<-max(P$`Total P`)+2

splot <- Pyr1
st <- '1'
mk <- mk1
png(file="TP1_Pmax_all.png", units="in",width=5, height=5.5, res=400)
...
plot(splot$Year,splot$yrmean, las=1, ylim=c(0,yhigh),ylab="Max May-Oct TP (ppb)",xlab="Year")  
