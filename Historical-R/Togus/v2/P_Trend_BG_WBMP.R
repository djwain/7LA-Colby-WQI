...
npts <- PB %>% group_by(Month) %>% summarize(npts = n())

# Use June - September data
P <- PB %>% filter(Month == 7 | Month == 8 | (Month == 9 & Day <= 24) )
...
st <- '1'
mk <- mk1
png(file="TP1_P_BG.png", units="in",width=5, height=5.5, res=400)
...
plot(splot$Year,splot$yrmean, las=1, ylim=c(0,yhigh),ylab="Average 7/1-9/24 TP - BG (ppb)",xlab="Year")  
