...
Syr1 <- Savgd %>% filter(STATION == 2) %>% group_by(YEAR) %>% summarize(zs10 = as.numeric(quantile(`SECCHI DEPTH`, c(0.10), na.rm = TRUE)))
...
sdp <- ggplot(Syr1, aes(YEAR, zs10)) + 
  ...
  labs(title = 'LPDEP2 10%-ile Secchi ')
...
dev.print(png,paste(filepathC,"LP2 10thpc Secchi.png"),width = 682, res = 150)
