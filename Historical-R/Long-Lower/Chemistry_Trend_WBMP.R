...
st <- '2'
mk <- mk1
png(file="LP2_Col.png", units="in",width=5, height=5.5, res=400)
...
st <- '2'
mk <- mk10
png(file="LP2_Col_10.png", units="in",width=5, height=5.5, res=400)
...
yhigh<-max(Colyr1$`Color (SPU)`)+1
png(file="LP2_Col_all.png", units="in",width=10, height=5, res=400)
plot(Colord$Year,Colord$`Color (SPU)`,ylim=c(0,yhigh),pch=21,col = 'azure3',bg = 'azure3',
     ylab="Color (SPU)",xlab="Year",title('Long Pond Station 2'))  
...
png(file="LP2_Col_box.png", units="in",width=5, height=5.5, res=400)

S1 <- Colord
# Make box plot by year and by site
p <- ggplot(data = S1, aes(x = Dec,y = `Color (SPU)`)) +
  geom_boxplot() +
  labs(title = "Long Pond St. 2 Color by Decade", y = 'Color (SPU)',x = '')
print(p)
dev.off()
...
# Plot data and trendline for each
yhigh<-max(Condyr1$`Conductivity (uS)`)+6


splot <- Condyr1
st <- '2'
mk <- mk1
png(file="LP2_Cond.png", units="in",width=5, height=5.5, res=400)
...
png(file="LP2_Cond_box.png", units="in",width=5, height=5.5, res=400)

S1 <- Condd
# Make box plot by year and by site
p <- ggplot(data = S1, aes(x = Dec,y = `Conductivity (uS)`)) +
  geom_boxplot() +
  labs(title = "Long Pond St. 2 Conductivity by Decade", y = 'Conductivity (uS)',x = '')
print(p)
dev.off()
...
st <- '2'
mk <- mk1
png(file="LP2_Alk.png", units="in",width=5, height=5.5, res=400)
...
png(file="LP2_Alk_box.png", units="in",width=5, height=5.5, res=400)

S1 <- Alkd
# Make box plot by year and by site
p <- ggplot(data = S1, aes(x = Dec,y = `Alkalinity (mg/L)`)) +
  geom_boxplot() +
  labs(title = "Long Pond St. 2 Alkalinity by Decade", y = 'Alkalinity (mg/L)',x = '')
print(p)
dev.off()