...
# Use June-Sept
S <- Secchi %>% filter(MONTH >= 5 & MONTH <= 10 & STATION == 2)
S <- na.omit(S)
S <- distinct(S)
...
ggplot(Syr, aes(YEAR, ft)) + 
  geom_point() +
  labs(title = "Secchi Depth", y = "SDT (ft)", x = "Year") +
  geom_smooth(method='loess') +
  scale_y_continuous(trans = 'reverse', lim = c(9,0)*3.28) +
  labs(title = 'Average Lower Long Pond Secchi')
 # labs(title = 'GP1 Secchi, May-Oct Avg, tau = -0.18, p = 0.083')
dev.print(png,paste(filepath,"LP2 Avg Secchi.png"), res = 300, width = 1000)

S10 <- filter(Syr,YEAR >= 2010)
...
ggplot(S10, aes(YEAR, yrmean)) + 
  ...
  #geom_smooth(method='loess') +
  scale_y_continuous(trans = 'reverse', lim = c(9,3)) +
  labs(title = 'GP1 Secchi, May-Oct Avg, tau = -0.2, p = 0.474')

mk<-MannKendall(Smin$yrmin)


ggplot(Smin, aes(YEAR, yrmin*3.28)) + 
  geom_point() +
  labs(title = "Secchi Depth", y = "SDT (ft)", x = "Year") +
  geom_smooth(method='loess') +
  scale_y_continuous(trans = 'reverse', lim = c(9,0)*3.28) +
  labs(title = 'Minimum Lower Long Pond Secchi')
  #labs(title = 'GP1 Secchi, May-Oct Min, tau = -0.32, p = 0.0017')
dev.print(png,paste(filepath,"LP2 Min Secchi.png"), res = 300, width = 1000)

S10 <- filter(Smin,YEAR >= 2010)
...
ggplot(S10, aes(YEAR, yrmin)) + 
  ...
  labs(title = "Secchi Depth", y = "zS (m)", x = "Year") +
  #geom_smooth(method='loess') +
  scale_y_continuous(trans = 'reverse', lim = c(10,2)) +
  labs(title = 'GP1 Secchi, May-Oct Avg, tau = -0.33, p = 0.21')
