...
sdp10 <- ggplot(S10, aes(YEAR, ft)) + 
  geom_point() +
  labs(title = "Secchi Depth", y = "SDT (ft)", x = "Year") +
  geom_smooth(method='loess') +
  scale_y_continuous(trans = 'reverse', lim = c(10,0)*3.28) +
  labs(title = 'Average Great Pond Secchi Disk Transparency over Past 10 Years')
sdp10 <- sdp10 + geom_hline(yintercept = 13.1, color = "red")
sdp10 + geom_hline(yintercept = 26.2, color = "green")
