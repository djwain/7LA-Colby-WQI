...
ggplot(Syr, aes(YEAR, ft)) + 
  ...
  labs(title = 'Average Salmon Lake Secchi')
 # labs(title = 'GP1 Secchi, May-Oct Avg, tau = -0.18, p = 0.083')
dev.print(png,paste(filepath,"SL Avg Secchi.png"), res = 300, width = 1000)
...
ggplot(Smin, aes(YEAR, yrmin*3.28)) + 
  ...
  geom_smooth(method='loess') +
  scale_y_continuous(trans = 'reverse', lim = c(9,0)*3.28) +
  labs(title = 'Minimum Salmon Lake Secchi')
  #labs(title = 'GP1 Secchi, May-Oct Min, tau = -0.32, p = 0.0017')
dev.print(png,paste(filepath,"SL Min Secchi.png"), res = 300, width = 1000)
