...
st <- '1'
mk <- mk1
png(file="LP1_CHLA.png", units="in",width=5, height=5.5, res=400)
...
# Plot data and trendline for each
yhigh<-max(Cavg10$CHLA)

splot <- Cavg10
st <- '1'
mk <- mk10
png(file="LP1_CHLA_10.png", units="in",width=5, height=5.5, res=400)
...
png(file="LP1_CHL_all.png", units="in",width=10, height=5, res=400)
plot(Cavgd$Year,Cavgd$CHLA,ylim=c(0,yhigh),pch=21,col = 'azure3',bg = 'azure3',
     ylab="Chl-a (ppb)",xlab="Year",title('Long Pond Station 1'))  
...
png(file="LP1_CHL_box.png", units="in",width=5, height=5.5, res=400)

S1 <- Cavgd
# Make box plot by year and by site
p <- ggplot(data = S1, aes(x = Dec,y = CHLA)) +
  geom_boxplot() +
  labs(title = "Long Pond St. 1 Chlorophyll-a by Decade", y = 'Chl-a (ppb)',x = '')
print(p)
dev.off()
