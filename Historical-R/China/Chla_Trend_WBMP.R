...
yhigh<-max(Cyr1$yrmed)+1

splot <- Cyr1
st <- '1'
mk <- mk1
png(file="China1_CHLA_all.png", units="in",width=5, height=5.5, res=400)
...
# Compute year average by station
Cyr2 <- Cavgd %>% filter(Station == 2) %>% group_by(Year) %>% summarise(yrmed = median(CHLA))

# Do Mann Kendall trend analysis on whole time series
mk1<-MannKendall(Cyr2$yrmed)

# Plot data and trendline for each
yhigh<-max(Cyr2$yrmed)+1

splot <- Cyr2
st <- '2'
mk <- mk1
png(file="China2_CHLA_all.png", units="in",width=5, height=5.5, res=400)
...
# Compute year average by station
Cyr3 <- Cavgd %>% filter(Station == 3) %>% group_by(Year) %>% summarise(yrmed = median(CHLA))

# Do Mann Kendall trend analysis on whole time series
mk1<-MannKendall(Cyr3$yrmed)

# Plot data and trendline for each
yhigh<-max(Cyr3$yrmed)+1

splot <- Cyr3
st <- '3'
mk <- mk1
png(file="China3_CHLA_all.png", units="in",width=5, height=5.5, res=400)
