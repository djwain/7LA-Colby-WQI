...
# Use April 15-Oct 15?
S <- Secchi %>% filter(YDAY >= 105 & YDAY <= 288 & YEAR >= 2011)
S <- na.omit(S)
S <- distinct(S)
...
st <- '1'
mk <- mk1
png(file="China1_SDT_10.png", units="in",width=5, height=5.5, res=400)
...
st <- '2'
mk <- mk1
png(file="China2_SDT_10.png", units="in",width=5, height=5.5, res=400)
...
st <- '3'
mk <- mk1
png(file="China3_SDT_10.png", units="in",width=5, height=5.5, res=400)
