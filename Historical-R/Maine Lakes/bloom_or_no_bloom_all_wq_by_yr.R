...
fit0 <- rpart(yn ~ Area..acres. + Perimeter..miles. + Depth_Mean..feet. 
              ...
              + TWALA + OI + Ts + Tb + zT +zhypox+zanox+Ps+Pb
              + strat_on +strat_off+strat_days+hypox_on+hypox_off+hypox_days
              +anox_on +anox_off+anox_days, data=na.omit(bloom))
...
bloom2 <- select(bloom, Area..acres., Perimeter..miles., Depth_Mean..feet., 
                 ...
                 Ts, Tb, zT, zhypox, zanox, Ps, Pb, strat_on, strat_off, strat_days,
                 hypox_on, hypox_off, hypox_days, anox_on, anox_off, anox_days, yn)
fit1 <- ctree(yn ~ Area..acres. + Perimeter..miles. + Depth_Mean..feet. 
              + Depth_Max..feet. + Volume..acrefeet. + Direct.Drainage.Area..sq.miles.
              + Total.Drainage.Area..sq.miles. + Flushing.Rate..times.yr. + DWALA
              + TWALA + OI + Ts + Tb + zT +zhypox+zanox+Ps+Pb
              + strat_on +strat_off+strat_days+hypox_on+hypox_off+hypox_days
              +anox_on +anox_off+anox_days, data=na.omit(bloom2))
plot(fit1)
...
fit3 <- randomForest(yn ~ Area..acres. + Perimeter..miles. + Depth_Mean..feet. 
                    ...
                    + TWALA + OI + Ts + Tb + zT + zhypox +zanox + Ps +Pb
                    + strat_on +strat_off+strat_days+hypox_on+hypox_off+hypox_days
                    +anox_on +anox_off+anox_days, data=na.omit(train))
pred = predict(fit3, newdata=test[-28])
cm = table(test[,28], pred)
cm
varImpPlot(fit3, sort=TRUE)

tmp <- as.data.frame(colSums(is.na(bloom2)))
